...
class Room{    
    ...
    updateEntity(data){
        for (let i = 0; i < this.entities.length; i++){
            if (this.entities[i].uid === data.uid){
                this.entities[i].hp = data.hp;
                this.entities[i].ac = data.ac;
                break;
            }
        }
    ...
    initCombatOrder(client){
        client.send('initiation-order', this.initiationOrder);
    }
    ...
    roll(dice, client){
        const res = client.roll(dice);
        
        client.send('roll', res);
    ...
    addClient(client){
        ...
        for (let i = 0; i < this.clients.length; i++){
            if (client.characterUid === this.clients[i].characterUid){
        ...
        if (isNewCharacter){
            ...
            if (this.initiationOrder.length){
                client.send('initiation-order', this.initiationOrder);
            }
        }else{
            client.send('rejected', "Failed to join the campaign. You already joined with a different tab or window.");
        }
    }
